...
void initialise()
{
    ...
    g_projection_matrix = glm::ortho(-6.5f, 6.5f, -5.625f, 5.625f, -1.0f, 1.0f);
...
void update()
{
    ...
    if ( g_current_scene->m_state.player->get_position().x > LEVEL1_LEFT_EDGE) {
        g_view_matrix = glm::translate(g_view_matrix, glm::vec3(-g_current_scene->m_state.player->get_position().x, 5.625 / 2 + 1.75f, 0));
    } else {
        g_view_matrix = glm::translate(g_view_matrix, glm::vec3(-6.5, 5.625 / 2 + 1.75f, 0));
    }
