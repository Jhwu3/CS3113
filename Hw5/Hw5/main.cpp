...
{
    int health = g_current_scene->m_number_of_lives;
    g_current_scene = scene;
    g_current_scene->m_number_of_lives = health;
...
void initialise()
{
    ...
    // ————— LEVEL A SETUP ————— //
    g_levelA = new LevelA();
    g_levelA->m_number_of_lives = 3;
    g_levelB = new LevelB();
    g_levelC = new LevelC();
    g_MenuScreen = new MenuScreen();
    g_MenuScreen->m_number_of_lives = 3;
       
    g_levels[0] = g_levelA;
    g_levels[1] = g_levelB;
    g_levels[2] = g_MenuScreen;
    g_levels[3] = g_levelC;

    // Start at level A
    g_current_scene = g_levels[2];
...
void process_input()
{
    ...
    while (SDL_PollEvent(&event))
    {
        ...
        case SDL_KEYDOWN:
            ...
            case SDLK_SPACE:
                // Jump
                    if (!inMenu && g_current_scene->m_state.player->m_collided_bottom)
                {
...
//    if (!inMenu && glm::length(g_current_scene->m_state.player->get_movement()) > 1.0f)
//    {
...
void update()
{
    ...
    if (!inMenu && g_current_scene->m_state.player->get_position().x > LEVEL1_LEFT_EDGE) {
